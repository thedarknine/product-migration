...
def init_file():
    try:
        ...
        log_format = '%(asctime)s [%(levelname)s] %(message)s - %(filename)s / %(funcName)s()  (L.%(lineno)d)'
        logging.basicConfig(filename=log_file, level=logging.DEBUG, datefmt='%Y-%m-%d %H:%M:%S', format=log_format)
